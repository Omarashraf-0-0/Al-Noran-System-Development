...
import { X, FileText, Download, User, Globe, Box, Calendar, CheckCircle, AlertTriangle, Eye, Loader2 } from 'lucide-react';
...
const AcidConfirmationModal = ({
  ...
  const theme = {
    ...
    // Fetch fresh presigned URL before viewing
					...
					<div className="space-y-3">
						...
						{confirmData.uploads?.length > 0 && (
							...
							{confirmData.uploads.map((file, idx) => {
								const isLoading = loadingDoc === (file._id || file.id);
								return (
								<button 
									key={idx}
									onClick={() => handleViewDocument(file)}
									disabled={isLoading}
									className="flex items-center gap-2 text-xs text-[#1ba3b6] hover:underline w-full py-1 disabled:opacity-50"
								>
