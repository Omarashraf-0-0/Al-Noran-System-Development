    ...
    return (
        <div className={bubbleClasses}>
            ...
            <div className={textClasses}>
                <p>{message.text}</p>
                <span className={`text-xs mt-1 block ${isOwnMessage ? 'text-red-100 text-right' : 'text-gray-500 text-left'}`}>{message.timestamp}</span>
            </div>
        </div>
    );
};

const ChatWindow = ({ user, messages, onSendMessage, sending, currentUserId }) => {
      ...
      <div className="flex items-center gap-4 p-3 border-b border-gray-200 bg-gray-50">
        ...
        <div>
          ...
          <div className="flex items-center gap-2">
            ...
            <p className="text-sm text-gray-500">{user.status === 'active' ? 'نشط' : user.status === 'pending' ? 'في الانتظار' : 'منتهي'}</p>
          </div>
        </div>
      </div>
