...
const MessageBubble = ({ message, isOwnMessage, avatarUrl }) => {
    ...
    const textClasses = `
      p-3 rounded-2xl
      ${isOwnMessage ? 'bg-[#690000] rounded-br-none text-white' : 'bg-gray-200 text-gray-800 rounded-bl-none'}
    `;
    ...
    return (
        <div className={bubbleClasses}>
            ...
            <div className={textClasses}>
                <p>{message.text}</p>
                <span className={`text-xs mt-1 block ${isOwnMessage ? 'text-white text-left' : 'text-gray-500 text-right'}`}>{message.timestamp}</span>
            </div>
        </div>
    );
};
        ...
        <form onSubmit={handleSubmit} className="flex items-center gap-4">
          <input
            ...
            placeholder="اكتب رسالة"
                className="flex-1 bg-white text-gray-900 px-4 py-3 rounded-full border border-gray-300 focus:outline-none focus:ring-2 focus:ring-[#690000] transition"
          />
          <button type="submit" className="p-3 bg-[#690000] rounded-full text-white hover:bg-[#530000] transition-colors disabled:bg-gray-400" disabled={!newMessage.trim()}>
