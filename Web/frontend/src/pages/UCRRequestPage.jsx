						...
						<form onSubmit={handleSubmit} className="space-y-6">
							...
							<div className={`p-6 rounded-2xl border mb-6 transition-all ${isDarkMode ? "bg-white/5 border-white/10" : "bg-gray-50 border-gray-200"}`}>
								...
								<div className="mb-4">
									...
									<div className="flex flex-col sm:flex-row gap-4">
										<label className={`flex items-center gap-3 cursor-pointer p-4 border rounded-lg transition-all ${formData.certificationType === "noran"
											? isDarkMode ? "bg-green-900/30 border-green-600" : "bg-green-50 border-green-300"
											: isDarkMode ? "bg-white/5 border-white/10 hover:bg-white/10" : "bg-white border-gray-200 hover:bg-green-50"
											}`}>
											<input
												type="radio"
												...
												className="hidden"
											/>
											<span className="w-4 h-4 bg-green-500 rounded-full flex-shrink-0 shadow-md"></span>
											<div>
												<span className={`font-medium ${isDarkMode ? "text-gray-100" : "text-gray-800"}`}>على بطاقة الشركة (النوران)</span>
												<p className={`text-xs ${isDarkMode ? "text-gray-400" : "text-gray-500"}`}>
													النوران تتولى المستندات - رسوم 10%
												</p>
											</div>
										</label>
										<label className={`flex items-center gap-3 cursor-pointer p-4 border rounded-lg transition-all ${formData.certificationType === "client"
											? isDarkMode ? "bg-yellow-900/30 border-yellow-600" : "bg-yellow-50 border-yellow-300"
											: isDarkMode ? "bg-white/5 border-white/10 hover:bg-white/10" : "bg-white border-gray-200 hover:bg-yellow-50"
											}`}>
											<input
												type="radio"
												name="certificationType"
												...
												className="hidden"
											/>
											<span className="w-4 h-4 bg-yellow-500 rounded-full flex-shrink-0 shadow-md"></span>
											<div>
												<span className={`font-medium ${isDarkMode ? "text-gray-100" : "text-gray-800"}`}>على بطاقتي الخاصة</span>
												<p className={`text-xs ${isDarkMode ? "text-gray-400" : "text-gray-500"}`}>
								...
								{formData.shippingMethod === "sea" && (
									<div className={`mb-4 p-4 rounded-lg border transition-all ${isDarkMode ? "bg-blue-900/20 border-blue-800/30 text-blue-200" : "bg-blue-50 border-blue-200 text-blue-900"}`}>
										...
										<div className="grid grid-cols-1 md:grid-cols-3 gap-3">
											...
											<label className={`flex flex-col items-center justify-center text-center cursor-pointer p-4 border-2 rounded-lg transition-colors ${formData.seaShipmentType === "fcl"
												? "border-blue-500 bg-blue-50"
												: isDarkMode ? "bg-gray-800 border-gray-600 hover:bg-gray-700" : "bg-white border-gray-200 hover:bg-blue-100"
												}`}>
												...
												<span className={`font-semibold ${formData.seaShipmentType === "fcl" ? "text-gray-800" : isDarkMode ? "text-gray-100" : "text-gray-800"}`}>حاويات</span>
												<span className="text-sm text-blue-600 font-medium">(FCL)</span>
												<p className={`text-xs mt-1 ${isDarkMode && formData.seaShipmentType !== "fcl" ? "text-gray-400" : "text-gray-500"}`}>Full Container Load</p>
												<p className={`text-xs ${isDarkMode && formData.seaShipmentType !== "fcl" ? "text-gray-500" : "text-gray-400"}`}>حاوية كاملة أو أكثر</p>
											</label>

											{/* LCL - Less than Container Load */}
											<label className={`flex flex-col items-center justify-center text-center cursor-pointer p-4 border-2 rounded-lg transition-colors ${formData.seaShipmentType === "lcl"
												? "border-blue-500 bg-blue-50"
												: isDarkMode ? "bg-gray-800 border-gray-600 hover:bg-gray-700" : "bg-white border-gray-200 hover:bg-blue-100"
												}`}>
												<div className="flex items-center gap-2 mb-2">
												...
												<span className={`font-semibold ${formData.seaShipmentType === "lcl" ? "text-gray-800" : isDarkMode ? "text-gray-100" : "text-gray-800"}`}>بضايع عامة</span>
												<span className="text-sm text-blue-600 font-medium">(LCL)</span>
												<p className={`text-xs mt-1 ${isDarkMode && formData.seaShipmentType !== "lcl" ? "text-gray-400" : "text-gray-500"}`}>Less than Container Load</p>
												<p className={`text-xs ${isDarkMode && formData.seaShipmentType !== "lcl" ? "text-gray-500" : "text-gray-400"}`}>أقل من حاوية كاملة</p>
											</label>

											{/* Parcels */}
											<label className={`flex flex-col items-center justify-center text-center cursor-pointer p-4 border-2 rounded-lg transition-colors ${formData.seaShipmentType === "parcels"
												? "border-blue-500 bg-blue-50"
												: isDarkMode ? "bg-gray-800 border-gray-600 hover:bg-gray-700" : "bg-white border-gray-200 hover:bg-blue-100"
												}`}>
												<div className="flex items-center gap-2 mb-2">
												...
												<span className={`font-semibold ${formData.seaShipmentType === "parcels" ? "text-gray-800" : isDarkMode ? "text-gray-100" : "text-gray-800"}`}>طرود</span>
												<p className={`text-xs mt-1 ${isDarkMode && formData.seaShipmentType !== "parcels" ? "text-gray-400" : "text-gray-500"}`}>شحن بالطرود والكراتين</p>
												<p className={`text-xs ${isDarkMode && formData.seaShipmentType !== "parcels" ? "text-gray-500" : "text-gray-400"}`}>بضاعة منفصلة</p>
											</label>
										</div>
									</div>
								)}
							...
							<div className={`p-6 rounded-2xl border mb-6 transition-all ${isDarkMode ? "bg-white/5 border-white/10" : "bg-gray-50 border-gray-200"}`}>
								...
								{items.map((item, index) => (
									<div
										key={index}
										className={`p-4 rounded-lg border mb-4 ${index === 0
									...
									>
										<div className="flex justify-between items-center mb-3">
											<div className="flex items-center gap-2">
												<span className={`font-medium ${isDarkMode ? "text-red-400" : "text-[#690000]"}`}>
													البند {index + 1}
												</span>
												{index === 0 && (
										...
										<div className="grid grid-cols-1 md:grid-cols-3 gap-3">
											<div>
												...
												<input
													...
													className={`w-full p-2 shadow border rounded-2xl focus:outline-none focus:ring-2 focus:ring-[#690000]/50 ${isDarkMode ? "bg-gray-700 text-white border-gray-600 placeholder-gray-400" : "bg-white text-black border-gray-200"}`}
													dir="rtl"
													required={index === 0}
												/>
											</div>
											<div>
												<label className={`text-xs mb-1 block whitespace-nowrap ${isDarkMode ? "text-gray-300" : "text-gray-600"}`}>
							...
							<div
								...
								className={`p-4 rounded-lg border ${documentsNeedingRevision.length > 0
									? "border-orange-400 ring-2 ring-orange-300 animate-pulse bg-orange-50 dark:bg-orange-900/10"
							...
							>
								...
								<div className={`mb-4 p-3 rounded-lg border transition-all ${isDarkMode ? "bg-blue-900/20 border-blue-800/30" : "bg-blue-50 border-blue-200"}`}>
									<div className="flex items-center gap-2">
										<span
											className={`w-4 h-4 rounded-full ${formData.certificationType === "noran"
												? "bg-green-500"
												: "bg-yellow-500"
												}`}
										></span>
								...
								<div className="grid grid-cols-1 md:grid-cols-3 gap-4">
									{getRequiredDocuments().map((docType, index) => {
										...
										return (
											<div
												key={docType}
												className={`relative border-2 rounded-xl p-4 transition-all ${needsRevision
											...
											>
												...
												<div className="text-center mb-3">
													<div className="flex justify-center gap-2 mb-2">
														<span className={`inline-flex items-center justify-center w-8 h-8 rounded-full text-white text-sm font-bold ${needsRevision ? "bg-orange-500" : isUploaded ? "bg-green-500" : isRequired ? "bg-red-600" : "bg-gray-400"
															}`}>
										...
										<div className="flex gap-1">
											{getRequiredDocuments().map((docType, index) => {
												...
												return (
													<span
														key={docType}
														className={`w-3 h-3 rounded-full ${isUploaded
															? "bg-green-500"
															: index === 0
																? "bg-red-400"
																: "bg-gray-300"
