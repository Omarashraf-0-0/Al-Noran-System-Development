...
const AdminProfilePage = () => {
	...
	const theme = {
		pageBg: isDarkMode 
			? "bg-[#0f0c05]" // Very dark gold/brown tint
			: "bg-[#F9F7F2]", // Warm off-white
		headerBg: isDarkMode
			? "bg-gradient-to-r from-[#B8860B] to-[#5c4005]" // Dark Gold Gradient
			: "bg-gradient-to-r from-[#DAA520] to-[#B8860B]", // Gold Gradient
		card: isDarkMode 
			? "bg-[#14120a] border-[#3d3215]" 
			: "bg-white border-amber-50 shadow-sm",
		textPrimary: isDarkMode ? "text-amber-50" : "text-gray-900",
		textSecondary: isDarkMode ? "text-amber-200/60" : "text-gray-500",
		input: isDarkMode 
			? "bg-[#2b2410] border-[#4a3f1a] text-amber-50 focus:border-amber-500" 
			: "bg-amber-50/50 border-amber-200/50 text-gray-900 focus:border-amber-500",
		buttonPrimary: "bg-gradient-to-r from-[#DAA520] to-[#B8860B] text-white hover:shadow-lg hover:shadow-amber-900/40",
		buttonSecondary: isDarkMode 
			? "bg-[#2b2410] text-amber-200 hover:bg-[#3d3215]" 
			: "bg-white text-amber-800 hover:bg-amber-50 border border-amber-200",
		iconBg: isDarkMode ? "bg-amber-500/10 text-amber-400" : "bg-amber-50 text-amber-600",
	...
	return (
		<div className={`min-h-screen ${theme.pageBg} relative overflow-hidden transition-colors duration-300 font-sans`} dir="rtl">
			...
			<div className="fixed inset-0 pointer-events-none overflow-hidden">
				{isDarkMode ? (
					<>
						<div className="absolute top-[5%] left-[5%] w-[500px] h-[500px] bg-[#DAA520]/10 rounded-full filter blur-[150px] animate-pulse-glow"></div>
						<div className="absolute bottom-[10%] right-[10%] w-[600px] h-[600px] bg-[#B8860B]/10 rounded-full filter blur-[180px] animate-float-slow"></div>
						<div className="absolute top-[40%] right-[20%] w-[400px] h-[400px] bg-[#CD853F]/10 rounded-full filter blur-[120px] animate-float-reverse"></div>
						<div className="absolute top-[10%] right-[10%] w-20 h-20 border-2 border-[#DAA520]/20 rounded-xl animate-float rotate-45"></div>
						<div className="absolute bottom-[20%] left-[8%] w-16 h-16 border-2 border-[#B8860B]/20 rounded-full animate-float-reverse"></div>
					</>
				) : (
					<>
						<div className="absolute top-[5%] left-[5%] w-[500px] h-[500px] bg-[#DAA520]/5 rounded-full filter blur-[150px] animate-pulse-glow"></div>
						<div className="absolute bottom-[10%] right-[10%] w-[600px] h-[600px] bg-[#F0E68C]/20 rounded-full filter blur-[180px] animate-float-slow"></div>
						<div className="absolute top-[40%] right-[20%] w-[400px] h-[400px] bg-[#DAA520]/5 rounded-full filter blur-[120px] animate-float-reverse"></div>
						<div className="absolute top-[10%] right-[10%] w-20 h-20 border-2 border-[#DAA520]/20 rounded-xl animate-float rotate-45"></div>
						<div className="absolute bottom-[20%] left-[8%] w-16 h-16 border-2 border-[#B8860B]/20 rounded-full animate-float-reverse"></div>
					</>
				)}
			...
			<main className="relative pt-28 pb-12 px-4 md:px-6 max-w-7xl mx-auto space-y-8">
					...
					<div className="px-6 md:px-10 pb-8">
						<div className="flex flex-col md:flex-row gap-6 mt-[-3rem] md:mt-[-4rem] items-center md:items-end">
									...
									<div className="w-full h-full rounded-full overflow-hidden relative bg-gray-100">
										{loadingPhoto || uploadingPhoto ? (
											<div className="w-full h-full flex items-center justify-center bg-gray-100">
												<div className="animate-spin rounded-full h-8 w-8 border-2 border-[#DAA520] border-t-transparent"></div>
											</div>
										) : profilePhoto ? (
							...
							<div className="flex items-center gap-3">
								<button 
									onClick={toggleTheme}
									className={`p-3 rounded-full transition-all ${isDarkMode ? "bg-amber-400/10 text-amber-400 hover:bg-amber-400/20" : "bg-gray-100 text-gray-600 hover:bg-gray-200"}`}
								>
									{isDarkMode ? "☀️" : "🌙"}
								</button>
								{isEditing ? (
				...
				<div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
					{[
						{ label: "المستخدمين", value: "84", icon: "👥", color: "text-[#DAA520]", link: "/customermanagement" },
						{ label: "الشحنات", value: "256", icon: "📦", color: "text-blue-500", link: "/shipmentsmanagement" },
						{ label: "الموظفين", value: "12", icon: "👔", color: "text-green-600", link: "/employeemanagement" },
						{ label: "تقارير", value: "8", icon: "📊", color: "text-purple-600", link: "/admindashboard" },
					].map((stat, idx) => (
					...
					<div className="space-y-6">
						...
						<div className={`rounded-3xl ${theme.card} border p-6`}>
							...
							<div className="space-y-4">
								<div className={`p-4 rounded-xl flex items-center justify-between ${isDarkMode ? "bg-amber-500/10 border border-amber-500/20" : "bg-amber-50 border border-amber-200"}`}>
									<div className="flex items-center gap-3">
										<div className={`w-10 h-10 rounded-full flex items-center justify-center ${isDarkMode ? "bg-amber-500/20 text-amber-500" : "bg-amber-100 text-amber-600"}`}>
											🔑
										</div>
										<div>
											<p className={`font-bold text-sm ${theme.textPrimary}`}>حساب إداري</p>
											<p className={`text-xs ${theme.textSecondary}`}>لديك صلاحيات كاملة</p>
										</div>
									</div>
								</div>
							...
							{["currentPassword", "newPassword", "confirmPassword"].map((field, idx) => (
								<div key={idx}>
									...
									<div className="relative">
										...
										/>
										<button type="button" onClick={() => setShowPasswords(prev => ({ ...prev, [field]: !prev[field] }))} className="absolute left-3 top-3.5 text-gray-400 hover:text-[#DAA520]">
