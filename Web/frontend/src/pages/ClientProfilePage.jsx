...
const ClientProfilePage = () => {
	...
	const fetchDocuments = async () => {
		try {
			...
			const registrationDocs = (response.data.uploads || []).filter(
				upload => upload.documentType && upload.documentType !== 'profilePhoto'
			);
	...
	const handleCropConfirm = async () => {
		...
		try {
			...
			formData.append("clientType", user?.clientDetails?.clientType || "personal");
			...
			if (response.data.upload?.s3Key) {
				...
				const displayUrl = response.data.upload?.url || response.data.upload?.publicUrl;
	...
	const handleDeleteAllUploads = async () => {
		...
		if (!window.confirm(`هل أنت متأكد من حذف جميع الملفات (${documents.length} ملف)؟`)) {
			return;
		}
		...
		try {
			...
			const deletePromises = documents.map(doc => 
				axios.delete(
					`${import.meta.env.VITE_API_URL}/api/uploads/${doc._id}`,
					{
						headers: { Authorization: `Bearer ${token}` },
					}
				)
					...
					<div className="p-8">
						<div className="flex items-start gap-8">
							...
							<div className="flex flex-col items-center flex-shrink-0">
								...
								<svg className="w-20 h-20 text-cyan-700" fill="none" stroke="currentColor" viewBox="0 0 24 24">
									...
									{!loadingPhoto && (
										<>
											<label 
												...
												style={{ 
													backgroundColor: uploadingPhoto ? '#E0E0E0' : '#1BA3B6',
													color: 'white',
													borderColor: uploadingPhoto ? '#E0E0E0' : '#1BA3B6',
													cursor: uploadingPhoto ? 'not-allowed' : 'pointer'
												}}
												onMouseEnter={(e) => !uploadingPhoto && (e.target.style.backgroundColor = '#158A9A')}
												onMouseLeave={(e) => !uploadingPhoto && (e.target.style.backgroundColor = '#1BA3B6')}
											>
												{uploadingPhoto ? 'جاري الرفع...' : (profilePhoto ? 'تغيير الصورة' : 'إضافة صورة')}
											</label>
								...
								<p className="text-gray-700 font-semibold text-lg mt-3 text-center">{user?.fullname || "اسم العميل"}</p>
							</div>
									...
									<div className="relative">
										...
										<div className="relative">
											<input
												...
												readOnly={!isEditing}
												className={`w-full px-4 py-3 pl-10 border border-gray-300 rounded-lg text-right ${isEditing ? 'bg-white' : 'bg-gray-50'} text-gray-700`}
											/>
									...
									{!isEditing ? (
										<>
											<button
												...
												onMouseEnter={(e) => e.target.style.backgroundColor = '#158A9A'}
												onMouseLeave={(e) => e.target.style.backgroundColor = '#1BA3B6'}
											>
										...
										<>
											<button
												onClick={() => {
													if (hasUnsavedChanges) {
														if (window.confirm("لديك تغييرات غير محفوظة. هل تريد تجاهلها والإلغاء؟")) {
															setIsEditing(false);
											...
											<button
												...
												style={{ backgroundColor: '#1BA3B6' }}
												onMouseEnter={(e) => hasUnsavedChanges && (e.target.style.backgroundColor = '#158A9A')}
												onMouseLeave={(e) => hasUnsavedChanges && (e.target.style.backgroundColor = '#1BA3B6')}
											>
												<svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
						...
						>
							...
							<>
								<div className="space-y-4">
									{(() => {
										const clientType = user?.clientDetails?.clientType || "personal";
										const requiredDocs = documentRequirements[clientType] || [];
										
										return requiredDocs.map((docReq) => {
											const doc = documents.find(d => d.documentType === docReq.key);
											const hasDocument = !!doc;
											
											return (
												<div key={docReq.key} className="bg-white border border-gray-200 rounded-lg overflow-hidden">
													<div className="p-4">
														<div className="flex items-center justify-between">
															<div className="text-left">
																...
																<p className="text-sm text-gray-500">
																	{hasDocument ? `بتاريخ ${new Date(doc.uploadedAt).toLocaleDateString('ar-EG')}` : 'لم يتم الرفع بعد'}
																</p>
															</div>
							...
							<div>
								...
								<div className="relative">
									<input
										...
										className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-red-700 focus:border-transparent text-right"
									/>
									<button
										type="button"
										onClick={() =>
