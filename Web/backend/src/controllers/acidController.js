...
// ✅ إنشاء طلب ACID جديد
exports.createAcidRequest = async (req, res) => {
  try {
    ...
    console.error(error);
    res.status(500).json({ message: "Server error while creating ACID request" });
  }
};
...
exports.getRequestByAcid = async (req, res) => {
  try {
...
exports.updateAcidStatus = async (req, res) => {
  try {
    const { id } = req.params;
    ...
    console.error(error);
    res.status(500).json({ message: "Server error while updating ACID request" });
  }
};
